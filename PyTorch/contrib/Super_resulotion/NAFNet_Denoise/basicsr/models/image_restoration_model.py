...
# ------------------------------------------------------------------------
# Adapted to tecorigin hard

import importlib
import torch
from torch_sdaa.utils import cuda_migrate  # 自动迁移方法
...
class ImageRestorationModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        # 使用 torch.compile() 编译网络
        if opt.get('train', {}).get('use_compile', False):
            self.net_g = torch.compile(self.net_g, backend='teco_inductor')
        ...
        #初始化 GradScaler
        self.use_amp = opt['train'].get('use_amp', False) and torch.cuda.is_available()
