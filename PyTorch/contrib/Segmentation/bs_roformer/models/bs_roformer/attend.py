...
class Attend(nn.Module):
    def __init__(
        ...
        self.cuda_config = FlashAttentionConfig(False, True, False)  # 强制禁用 flash
        ...
        if hasattr(device_properties, 'major') and hasattr(device_properties, 'minor'):
            device_version = version.parse(f'{device_properties.major}.{device_properties.minor}')
        else:
            # 给出默认值或通过其他方式获取版本信息
            device_version = version.parse("0.0")  # 或根据 SDAA 情况给出一个合适的版本号
