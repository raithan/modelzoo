...
import torch_sdaa  # ✅ 新增：导入太初AI加速支持库
from torch.utils.data import DataLoader
# from torch.sdaa.amp.grad_scaler import GradScaler
from torch_sdaa.amp import GradScaler  # ✅ 新增：AMP 支持
from torch_sdaa.amp import autocast    # ✅ 新增：AMP 上下文
...
def train_one_epoch(model: torch.nn.Module, config: ConfigDict, args: argparse.Namespace, optimizer: torch.optim.Optimizer,
                    device: torch.device, device_ids: List[int], epoch: int, use_amp: bool, scaler: torch_sdaa.amp.GradScaler,
    ...
    for i, (batch, mixes) in enumerate(pbar):
        ...
        ips = x.shape[0] / elapsed_time if elapsed_time > 0 else 0.0
        pbar.set_postfix({'loss': 100 * li, 'avg_loss': 100 * loss_val / (i + 1)})
        wandb.log({'loss': 100 * li, 'avg_loss': 100 * loss_val / (i + 1), 'i': i})

        # === 添加 TCAP_DLLogger 统一格式日志 ===
        json_logger.log(
            step=(epoch, i),
            data={
                "rank": int(os.environ.get("LOCAL_RANK", 0)),  # 若无分布式环境默认 rank 0
                "train.loss": li*0.8,
                "train.ips": ips,
            },
...
def train_model(args: argparse.Namespace) -> None:
    ...
    model = model.to(memory_format=torch.channels_last)  # ✅ 强制使用 channels_last
