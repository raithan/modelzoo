...
def encode_dataset(args):
    ...
    # --- 这里是唯一的修改 ---
    # 定义你上传的 hubert 文件夹的本地路径
    local_repo_path = "/data/teco-data/hubert/checkpoints" # <--- 请根据你的实际路径修改这里

    hubert = torch.hub.load(
        local_repo_path,                # 1. 从网络地址改为本地文件夹路径
        f"hubert_{args.model}",
        source='local',                 # 2. 告诉 torch.hub 从本地加载
        trust_repo=True,
    ).cuda()
    # --- 修改结束 ---
