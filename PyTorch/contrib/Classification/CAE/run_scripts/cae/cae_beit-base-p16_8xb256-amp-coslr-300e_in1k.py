...
data_root = '/data/teco-data/imagenet/' ##数据集路径
...
train_dataloader = dict(
    batch_size=50, ##batch size两边需要一样的地方 如果cuda of out memory需要调小
    num_workers=8,
    ...
    dataset=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file='train_list.txt', ##根据情况
        data_prefix=dict(img_path=''), ##根据情况
...
# runtime settings
#train_cfg = dict(type='EpochBasedTrainLoop', max_epochs=300) ###原本是epoch 注释掉
train_cfg = dict(type='IterBasedTrainLoop', max_iters=100, val_interval=4000) ###现在是iter 
default_hooks = dict(
