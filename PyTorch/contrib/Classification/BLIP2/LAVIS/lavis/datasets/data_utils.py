...
def move_to_sdaa(sample):
    def _move_to_sdaa(tensor):
        return tensor.sdaa()

    return apply_to_sample(_move_to_sdaa, sample)


def prepare_sample(samples, sdaa_enabled=True):
    if sdaa_enabled:
        samples = move_to_sdaa(samples)
