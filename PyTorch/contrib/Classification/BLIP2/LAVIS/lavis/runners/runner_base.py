...
class RunnerBase:
    ...
    def optimizer(self):
        if self._optimizer is None:
            ...
            num_parameters = sum(p.data.nelement() for g in optim_params for p in g["params"])
    ...
    def scaler(self):
        ...
        if amp and self._scaler is None:
            self._scaler = torch.sdaa.amp.GradScaler()
        return self._scaler
    ...
    def dataloaders(self):
        if self._dataloaders is None:
            ...
            for split_name in self.datasets:
                ds_obj = self.datasets[split_name]
                if isinstance(ds_obj, (tuple, list)):
                    num_records = sum(
                        len(d)
                    ...
                    if hasattr(ds_obj, "__len__"):
                        num_records = len(ds_obj)
                    else:
                        num_records = -1
                        logging.info("Only a single wds.DataPipeline dataset, no __len__ attribute.")
                if num_records >= 0:
                    logging.info(f"Loaded {num_records} records for {split_name} split from the dataset.")
    ...
    @property
    def sdaa_enabled(self):
        return self.device.type == "sdaa"
    ...
    @property
    def extra_log_freq(self):
        return int(self.config.run_cfg.get("extra_log_freq", 100))

    @property
    def save_freq(self):
        return int(self.config.run_cfg.get("save_freq", 5))

    @property
    def val_freq(self):
        return int(self.config.run_cfg.get("val_freq", 1))

    @property
    def save_last(self):
        return int(self.config.run_cfg.get("save_last", True))
    ...
    def valid_splits(self):
        vs = self.config.run_cfg.get("valid_splits", [])
        if len(vs) == 0:
    ...
    def train_splits(self):
        ts = self.config.run_cfg.get("train_splits", [])
        if len(ts) == 0:
        ...
        for cur_epoch in range(self.start_epoch, self.max_epoch):
            if not self.evaluate_only:
                ...
                train_stats = self.task.train_epoch(
                    ...
                    sdaa_enabled=self.sdaa_enabled,
                    log_freq=self.log_freq,
                    accum_grad_iters=self.accum_grad_iters,
                    extra_log_freq=self.extra_log_freq,  # 传入新增参数
                )
        ...
        def _create_loader(dataset, num_workers, bsz, is_train, collate_fn):
            if isinstance(dataset, (ChainDataset, wds.DataPipeline)):
                ...
                loader = DataLoader(
                    ...
                    collate_fn=collate_fn,  # FIX: 传单个函数，不是整个列表
    ...
    def _save_checkpoint(self, cur_epoch, is_best=False):
        ...
        save_to = os.path.join(self.output_dir, f"checkpoint_{'best' if is_best else cur_epoch}.pth")
    ...
    def _load_checkpoint(self, url_or_filename):
        ...
        self.unwrap_dist_model(self.model).load_state_dict(checkpoint["model"])
