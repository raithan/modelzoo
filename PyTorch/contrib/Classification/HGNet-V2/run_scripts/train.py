...
def main():
    ...
    if use_amp == 'apex':
        ...
        if device.type in ('sdaa',) and amp_dtype == torch.float16:
        ...
        if args.naflex_patch_sizes:
            ...
            if args.naflex_patch_size_probs:
                if len(args.naflex_patch_size_probs) != len(args.naflex_patch_sizes):
                    from arguments import parser  # only needed here to keep error behavior
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if update_idx % args.log_interval == 0 or last_batch:
            ...
            if utils.is_primary(args):
                ...
                # 输出 TCAPPDLL 格式日志
                print(
                    f'TCAPPDLL {timestamp} - '
                    f'Epoch: {epoch} Iteration: {update_idx}  '
                    f'rank : {rank}  '
                    f'train.loss : {loss_val}  '
                    f'train.ips : {ips} imgs/s '
                    f'train.total_time : {total_time}'
                )
