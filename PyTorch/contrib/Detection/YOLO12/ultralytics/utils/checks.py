...
def check_amp(model):
    ...
    def amp_allclose(m, im):
        print("ampdevice", device)  # debug print
        ...
        import torch_sdaa  # scope for faster 'import ultralytics'
        with torch_sdaa.amp.autocast(enabled=True):
            b = m(batch, imgsz=imgsz, device=device, verbose=False)[0].boxes.data  # AMP inference
        del m
        
        # print("ampallclose", a.shape, b.shape)  # debug print
        # all_close = torch.allclose(a, b.float(), atol=0.5)
        # print("allclose result:", all_close)
    ...
    try:
        ...
        assert amp_allclose(YOLO("yolo11n.pt").to(device), im)
