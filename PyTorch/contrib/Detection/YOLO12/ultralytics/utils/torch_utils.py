...
def autocast(enabled: bool, device: str = "sdaa"):
    """
    ...
    Notes:
        ...
        - For older versions, it uses `torch.sdaa.autocast`.
    ...
    if TORCH_1_13:
        ...
        return torch.sdaa.amp.autocast(enabled)
...
def get_gpu_info(index):
    ...
    properties = torch.sdaa.get_device_properties(index)
...
def select_device(device="", batch=0, newline=False, verbose=True):
    ...
    Args:
        device (str | torch.device, optional): Device string or torch.device object. Options are 'None', 'cpu', or
            'sdaa', or '0' or '0,1,2,3'. Auto-selects the first available GPU, or CPU if no GPU is available.
    ...
    Examples:
        >>> select_device("sdaa:0")
        device(type='sdaa', index=0)

        >>> select_device("cpu")
        device(type='cpu')

    Notes:
        Sets the 'SDAA_VISIBLE_DEVICES' environment variable for specifying which GPUs to use.
    """
    ...
    for remove in "sdaa:", "none", "(", ")", "[", "]", "'", " ":
        device = device.replace(remove, "")  # to string, 'sdaa:0' -> '0' and '(0, 1)' -> '0,1'
    ...
    if cpu or mps:
        os.environ["SDAA_VISIBLE_DEVICES"] = "-1"  # force torch.sdaa.is_available() = False
    elif device:  # non-cpu device requested
        if device == "sdaa":
            device = "0"
        if "," in device:
            device = ",".join([x for x in device.split(",") if x])  # remove sequential commas, i.e. "0,,1" -> "0,1"
        visible = os.environ.get("SDAA_VISIBLE_DEVICES", None)
        os.environ["SDAA_VISIBLE_DEVICES"] = device  # set environment variable - must be before assert is_available()
        if not (torch.sdaa.is_available() and torch.sdaa.device_count() >= len(device.split(","))):
            LOGGER.info(s)
            install = (
                "See https://pytorch.org/get-started/locally/ for up-to-date torch install instructions if no "
                "SDAA devices are seen by torch.\n"
                if torch.sdaa.device_count() == 0
                else ""
            )
            raise ValueError(
                f"Invalid SDAA 'device={device}' requested."
                f" Use 'device=cpu' or pass valid SDAA device(s) if available,"
                f" i.e. 'device=0' or 'device=0,1,2,3' for Multi-GPU.\n"
                f"\ntorch.sdaa.is_available(): {torch.sdaa.is_available()}"
                f"\ntorch.sdaa.device_count(): {torch.sdaa.device_count()}"
                f"\nos.environ['SDAA_VISIBLE_DEVICES']: {visible}\n"
                f"{install}"
            )

    if not cpu and not mps and torch.sdaa.is_available():  # prefer GPU if available
        ...
        for i, d in enumerate(devices):
            s += f"{'' if i == 0 else space}SDAA:{d} ({get_gpu_info(i)})\n"  # bytes to MB
        arg = "sdaa:0"
...
def init_seeds(seed=0, deterministic=False):
    ...
    torch.sdaa.manual_seed_all(seed)  # for Multi-GPU, exception safe
...
def cuda_memory_usage(device=None):
    """
    Monitor and manage SDAA memory usage.

    This function checks if SDAA is available and, if so, empties the SDAA cache to free up unused memory.
    It then yields a dictionary containing memory usage information, which can be updated by the caller.
    Finally, it updates the dictionary with the amount of memory reserved by SDAA on the specified device.

    Args:
        device (torch.device, optional): The SDAA device to query memory usage for.
    ...
    if torch.sdaa.is_available():
        ...
        try:
            ...
            cuda_info["memory"] = torch.sdaa.memory_reserved(device)
    else:
        yield cuda_info
