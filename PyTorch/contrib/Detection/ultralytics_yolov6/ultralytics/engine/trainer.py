...
json_logger = Logger(
    [
        ...
        JSONStreamBackend(Verbosity.VERBOSE, "new_sdaa.log"),
    ]
)
    ...
    def _do_train(self, world_size=1):
        ...
        while True:
            ...
            for i, batch in pbar:
                ...
                if RANK in {-1, 0}:
                    #===========================================================
                    loss_value = self.loss.item() if hasattr(self, 'loss') and self.loss is not None else 0.0  # 改动1
                    if not hasattr(self, '_last_batch_time'):
                        self._last_batch_time = time.time()
                    cur_time = time.time()
                    batch_time = cur_time - self._last_batch_time
                    self._last_batch_time = cur_time
                    batch_size = batch["img"].shape[0] if "img" in batch else self.batch_size
                    train_ips = batch_size / batch_time if batch_time > 0 else 0
                    #=============================================================
                    ...
                    try:
                        json_logger.log(
                            ...
                            data={
                                ...
                                "train.ips": float(train_ips),
                            },
                            verbosity=Verbosity.DEFAULT,
                        )
                    except Exception as e:
                        print("Logging train batch error:", e)
                global_step += 1   # 步数递增
                #============================================
