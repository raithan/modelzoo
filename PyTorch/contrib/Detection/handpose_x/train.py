...
#加入了 tcap_dllogger 日志格式支持
from tcap_dllogger import Logger, StdOutBackend, JSONStreamBackend, Verbosity
json_logger = Logger([
    StdOutBackend(Verbosity.DEFAULT),
    JSONStreamBackend(Verbosity.VERBOSE, 'dlloger_output.json'),
])
    ...
    try:
        ...
        for epoch in range(0, ops.epochs):
            ...
            for i, (imgs_, pts_) in enumerate(dataloader):
                ...
                if i >= 99:  # 每个 epoch 最多 100 个 iteration
                    print(f"[INFO] Reached 100 iterations in epoch {epoch}, stopping early.")
                    break
