...
# 初始化 TCAP logger（建议在 main 函数开头只执行一次）
json_logger = Logger([
    StdOutBackend(Verbosity.DEFAULT),
    JSONStreamBackend(Verbosity.VERBOSE, 'dlloger_train_log.json')
])

# 添加训练阶段 log 的元信息（metadata）说明，只需初始化一次
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train(train_loader, model, criterion, optimizer, epoch, scaler, json_logger=json_logger)
...
def train(train_loader, model, criterion, optimizer, epoch, scaler, json_logger=None):
    ...
    for i, _ in enumerate(train_loader):
        ...
        if isinstance(output, (tuple, list)):
            loss_list = [criterion.forward(torch.sigmoid(out), target, epoch) for out in output]
            loss = sum(loss_list)
            # max-voting 聚合
            output = torch.max(torch.max(torch.max(output[0], output[1]), output[2]), output[3])
        else:
        ...
        # ========================== TCAP Logger 输出 ==========================
        ips = bs / batch_time.val  # instant images/sec
        json_logger.log(
            step=(epoch, i),
            data={
                "rank": int(os.environ.get("LOCAL_RANK", 0)),  # LOCAL_RANK 默认为 0
