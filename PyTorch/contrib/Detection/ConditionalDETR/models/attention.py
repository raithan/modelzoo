...
try:
    if torch.__version__.startswith('0.') or \
       (torch.__version__.startswith('1.') and int(torch.__version__.split('.')[1]) < 9):
...
try:
    from torch_sdaa.overrides import has_torch_function, handle_torch_function
except ImportError:
