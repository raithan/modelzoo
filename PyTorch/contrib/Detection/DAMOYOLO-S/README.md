...
1. 在构建好的环境中，进入文件目录。
    ```
    cd <ModelZoo_path>/PyTorch/contrib/Detection/DAMO-YOLO/scripts
    ```
2. 运行训练。该模型支持单机单卡。
    ```
    ...
    单机多卡
    python -m torch.distributed.launch --nproc_per_node=4 run_DAMO_YOLO_S.py -f configs/damoyolo_tinynasL25_S.py 2>&1 | tee sdaa.log  
   ```
