...
miscs = easydict({
    'print_interval_iters': 1,    # print interval
...
def get_config_by_file(config_file):
    try:
        # 1. 转为绝对路径，保证任何启动目录都能正确找到
        config_path = os.path.abspath(config_file)
        config_dir = os.path.dirname(config_path)
        module_name = os.path.basename(config_path).split('.')[0]
        
        # 2. 添加到sys.path最前
        sys.path.insert(0, config_dir)
        
        # 3. 导入
        current_config = importlib.import_module(module_name)
        exp = current_config.Config()
    except Exception as e:
        raise ImportError(
            "{} doesn't contains class named 'Config', original error: {}".format(config_file, e))
    return exp
