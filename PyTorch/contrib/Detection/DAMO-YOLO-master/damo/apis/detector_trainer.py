...
json_logger = Logger(
    [
        ...
        JSONStreamBackend(Verbosity.VERBOSE, "run_scripts/new_sdaa.log"),
    ]
)
...
class Trainer:
    ...
    def train(self, local_rank):
        ...
        for data_iter, (inps, targets, ids) in enumerate(self.train_loader):
            ...
            # === 【新增】结构化日志输出（每步） ===
            batch_size = inps.tensors.shape[0] if hasattr(inps, 'tensors') else inps.shape[0]
            ips = batch_size / (iter_end_time - iter_start_time + 1e-9)
            json_logger.log(
                step=(self.epoch + 1, data_iter),
                data={
