...
try:
    ...
    print("⚠️ tcap_dllogger not installed, training logs will be disabled.")
    json_logger = None
    Verbosity = None
...
class Trainer:
    ...
    def train_in_steps(self, epoch_num, step_num):
        ...
        start_time = time.time()  # 记录 step 开始时间
        ...
        # ✅ 计算 step 时间 & 训练吞吐率（images/s）
        step_time = time.time() - start_time
        batch_size = images.size(0)
        ips = batch_size / step_time if step_time > 0 else 0.0
        # ✅ 写入日志
        json_logger.log(
            step=(epoch_num, step_num),
            data={
        ...
        # ✅ 在100 step时停止训练
        if step_num >= 100:
            print(f"Reached 100 steps at epoch {epoch_num}, stopping training...")
            raise SystemExit(0)
