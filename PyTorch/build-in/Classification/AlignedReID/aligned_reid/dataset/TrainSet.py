...
class TrainSet(Dataset):
    ...
    def __init__(
        ...
        self.ids2labels = list(ids2labels.items())  
    ...
    def get_sample(self, ptr):
        ...
        """
        with self.lock:
            current_id = self.ids[ptr]
            inds = self.ids_to_im_inds[current_id]
            ...
            label = next((label for id, label in self.ids2labels if id == current_id), None)
            if label is None:
                raise ValueError(f"ID {current_id} not found in ids2labels")
            
            labels = [label for _ in range(self.ims_per_id)]
