...
def test(model, test_loader, btrain=False, model_file='model_92_sgd.pkl'):
    ...
    for images, labels in test_loader:
        images = Variable(images.sdaa())
        labels = Variable(labels.sdaa())
...
model = ResidualAttentionModel().sdaa()
print(model)
...
if is_train is True:
    ...
    for epoch in range(total_epoch):
        ...
        for i, (images, labels) in enumerate(train_loader):
            ...
            imgs, labels = images.to(device).to(memory_format=torch.channels_last), labels.to(device)     
