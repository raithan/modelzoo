...
import timm.data.dataset as Dataset
...
def main():
    ...
    def convert_to_splitbn(module, num_splits):
        """
        将模型中的所有 BatchNorm 层转换为 SplitBatchNorm 层
        参数:
            module: 输入的 PyTorch 模块
            num_splits: 分割数，用于 SplitBatchNorm
        返回:
            转换后的模块
        """
        if isinstance(module, nn.Sequential):
            module = nn.Sequential(*[convert_to_splitbn(m, num_splits) for m in module])
        elif isinstance(module, nn.BatchNorm2d):
            # 创建 SplitBatchNorm 层替代原始 BatchNorm
            new_bn = nn.SyncBatchNorm.convert_sync_batchnorm(module)
            new_bn.num_splits = num_splits
            return new_bn
        # 递归处理子模块
        for name, child in module.named_children():
            module.add_module(name, convert_to_splitbn(child, num_splits))
        return module
    ...
    if args.split_bn:
        ...
        model = convert_to_splitbn(model, max(num_aug_splits,2))
    ...
    if num_aug_splits > 1:
        ...
        transform = create_transform(
            ...
            is_training=True,
            auto_augment='augmix-m5-w4-d1'  # AugMix 风格增强策略
                    )
        dataset_train = Dataset(dataset_train, transform=transform)
