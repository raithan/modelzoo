...
def train_one_epoch(G: 'generator model', 
    ...
    for iteration, data in enumerate(dataloader):
        ...
        # 确保所有输入数据都是float32类型
        Xs_orig = Xs_orig.to(device).float()
        Xs = Xs.to(device).float()
        Xt = Xt.to(device).float()
        same_person = same_person.to(device).float()
...
if __name__ == "__main__":
    ...
    # dataset params
    parser.add_argument('--dataset_path', default='./vggface2_train/train/', help='Path to the dataset. If not VGG2 dataset is used, param --vgg should be set False')
