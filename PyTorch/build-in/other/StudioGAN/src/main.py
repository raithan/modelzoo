...
original_batch_norm = F.batch_norm

@functools.wraps(original_batch_norm)
def safe_batch_norm(input, running_mean, running_var, weight=None, bias=None, 
                   training=False, momentum=0.1, eps=1e-5):
    try:
        return original_batch_norm(input, running_mean, running_var, weight, bias, training, momentum, eps)
    except RuntimeError as e:
        if "Cannot access data pointer" in str(e):
            # print(f"Warning: Skipping BatchNorm operation due to storage access issue. Input shape: {input.shape}")
            return input
        else:
            raise e
        
F.batch_norm = safe_batch_norm
