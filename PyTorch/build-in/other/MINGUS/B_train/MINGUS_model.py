    ...
    for batch, i in enumerate(range(0, train_data_pitch.size(0) - 1, bptt)):
        ...
        if i == 0:
            ...
            if isPitch:
                ...
                # pitch_classes = data_pitch.cpu().detach().reshape(-1).numpy()
                pitch_classes = data_pitch.detach().cpu().contiguous().numpy().reshape(-1)
                ...
                dur_classes = data_duration.detach().cpu().contiguous().numpy().reshape(-1)
