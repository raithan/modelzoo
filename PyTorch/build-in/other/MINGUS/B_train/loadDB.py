...
def pad(data, isChord=False):
    ...
    for x in data:
        if isChord:
            # For chords, ensure each chord has exactly 4 notes
            processed_x = []
            for chord in x:
                if len(chord) < 4:
                    # Pad chord with '<pad>' notes to have 4 notes
                    padded_chord = list(chord) + ['<pad>'] * (4 - len(chord))
                else:
                    # Truncate to 4 notes if more than 4
                    padded_chord = list(chord[:4])
                processed_x.append(padded_chord)
            padded.append(
                 list(processed_x[:max_len])
                 + [pad_token] * max(0, max_len - len(processed_x)))
        else:
            padded.append(
                 #([init_token])
                 #+ 
...
def batchify(data, bsz, dict_to_ix, device, SEGMENTATION=True, isChord=False):
        ...
        if isChord:
            ...
            for sequence in padded:
                ...
                for chord in sequence:
                    chord_notes = []
                    for note in chord:
                        if note in dict_to_ix:
                            chord_notes.append(dict_to_ix[note])
                        else:
                            chord_notes.append(dict_to_ix['R'])
                    seq_chords.append(chord_notes)
                padded_num.append(seq_chords)
        else:
            padded = pad(data)
    ...
    try:
        ...
        print("Sample of padded_num:", padded_num[0][:5] if len(padded_num) > 0 else "Empty")
        raise e
        ...
        for n in seq_chord[i]:
            if n == 'R':
                ...
                try:
                    ...
                    ch.append('R')
        # Ensure chord has exactly 4 notes
        if len(ch) < 4:
            ch = ch + ['R'] * (4 - len(ch))
        else:
            ch = ch[:4]
        ...
        for n in seq_next_chord[i]:
            if n == 'R':
                ...
                try:
                    ...
                    nch.append('R')
        # Ensure next chord has exactly 4 notes
        if len(nch) < 4:
            nch = nch + ['R'] * (4 - len(nch))
        else:
            nch = nch[:4]
...
class MusicDB():
    ...
    def __init__(self, device, TRAIN_BATCH_SIZE, EVAL_BATCH_SIZE,
        ...
        # print("Number of sequences:", len(pitch_train))
        # print("Lengths of first 10 sequences:", [len(seq) for seq in pitch_train[:10]])
        # print("Any empty?", [len(seq) == 0 for seq in pitch_train[:10]])
        ...
        for song in chord_train:
            new_song_chords = []
            for chord in song:
                chord_notes = CustomToMidiChords[chord]
                if len(chord_notes) >= 4:
                    processed_chord = chord_notes[:4]
                else:
                    processed_chord = list(chord_notes) + ['R'] * (4 - len(chord_notes))
                new_song_chords.append(processed_chord)
        ...
        if AUGMENTATION:
            ...
            for aug in range(1, augmentation_const):
                for i in range(len(pitch_train)):               
                    ...
                    new_chord_aug = []
                    for chord in chord_train[i]:
                        new_chord_notes = []
                        for note in chord:
                            if note == 'R':
                                new_chord_notes.append(note)
                            else:
                                try:
                                    note_int = int(note)
                                    new_chord_notes.append(note_int + aug)
                                except (ValueError, TypeError):
                                    new_chord_notes.append(note)
                        new_chord_aug.append(new_chord_notes)
                    new_chord.append(new_chord_aug)
                    
                    new_next_chord_aug = []
                    for chord in next_chord_train[i]:
                        new_chord_notes = []
                        for note in chord:
                            if note == 'R':
                                new_chord_notes.append(note)
                                ...
                                try:
                                    ...
                                    new_chord_notes.append(note)
                        new_next_chord_aug.append(new_chord_notes)
                    new_next_chord.append(new_next_chord_aug)
            ...
            new_offset = []
            for aug in range(1, augmentation_const):
                for i in range(len(pitch_validation)):
                    ...
                    new_chord_aug = []
                    for chord in chord_validation[i]:
                        new_chord_notes = []
                        for note in chord:
                            if note == 'R':
                                new_chord_notes.append(note)
                    ...
                    new_next_chord_aug = []
                    for chord in next_chord_validation[i]:
                        new_chord_notes = []
                        for note in chord:
                            if note == 'R':
                                new_chord_notes.append(note)
            ...
            for aug in range(1, augmentation_const):
                for i in range(len(pitch_test)):
                    ...
                    new_next_chord_aug = []
                    for chord in next_chord_test[i]:
                        new_chord_notes = []
                        for note in chord:
                            if note == 'R':
                                new_chord_notes.append(note)
