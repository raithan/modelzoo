...
class SkyDetector():
    ...
    def _collect_running_batch_states(self):
        ...
        # 只在训练时打印每个step的loss
        if self.is_training:
            print(f'Epoch [{self.epoch_id}][{self.batch_id}/{m}], G_loss: {self.G_loss.item():.9f}')

        # 每100个batch打印一次详细信息（可选）
        # if np.mod(self.batch_id, 100) == 1 and self.is_training:
