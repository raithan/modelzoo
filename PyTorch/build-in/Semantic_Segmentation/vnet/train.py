...
def main():
    ...
    print("Images:", ct_images)
    print("Targets:", ct_targets)
    print("Number of images:", len(ct_images))
    print("Number of targets:", len(ct_targets))
...
def train_nll(args, epoch, model, trainLoader, optimizer, trainF, weights):
    ...
    for batch_idx, (data, target) in enumerate(trainLoader):
        ...
        # print("output shepe is:", output.shape)
        ...
        #     loss.item(), err, dice_loss))

        print('Step: {} | Train Epoch: {:.2f} | Loss: {:.4f} | Error: {:.3f}% | Dice: {:.6f}'.format(
                batch_idx, partialEpoch, loss_val, err, dice_loss))
...
def train_dice(args, epoch, model, trainLoader, optimizer, trainF, weights):
    ...
    for batch_idx, (data, target) in enumerate(trainLoader):
        ...
        print('Step: {} | Train Epoch: {:.2f} | Loss: {:.8f} | Error: {:.8f}'.format(
            batch_idx, partialEpoch, loss_val, err))

        trainF.write('{},{},{}\n'.format(partialEpoch, loss_val, err))
        trainF.flush()
