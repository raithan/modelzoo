...
def train_3d(config, model, optimizer, loader, epoch, output_dir, writer_dict, device=torch.device('cuda'), dtype=torch.float):
    ...
    if model.backbone is not None:
        model.backbone.eval()  # Comment out this line if you want to train 2D backbone jointly
    ...
    for i, (inputs, targets_2d, weights_2d, targets_3d, meta, input_heatmap) in enumerate(loader):
        ...
        # targets_2d = to_device(targets_2d, device)
        # weights_2d = to_device(weights_2d, device)
        # targets_3d = to_device(targets_3d, device)
        # meta = to_device(meta, device)
        # input_heatmap = to_device(input_heatmap, device)
        ...
        # if loss_cord > 0:
        #     # optimizer.zero_grad()
        #     # print(f"loss_2d dtype: {loss_2d.dtype}")
        #     # print(f"loss_cord dtype: {loss_cord.dtype}")
        #     # print(f"loss_2d + loss_cord dtype: {(loss_2d + loss_cord).dtype}")        
        #     scaler.scale(loss_2d + loss_cord).backward()
        ...
        #     # optimizer.zero_grad()
        #     # print(f"accu_loss_3d: {accu_loss_3d}")
        #     # print(f"accu_loss_3d dtype: {accu_loss_3d.dtype}")
        #     scaler.scale(accu_loss_3d).backward()
