...
def synthetic_train(config, synthetic_train_loader, real_train_loader, model, criterion, optimizer, epoch,
    ...
    for i, (input, target, target_weight, meta) in enumerate(synthetic_train_loader):
        ...
        input = input.to("sdaa")
        target = target.to("sdaa")
        target_weight = target_weight.to("sdaa")
...
def train(config, train_loader, model, criterion, optimizer, epoch,
    ...
    for i, (input, target, target_weight, meta) in enumerate(train_loader):
        ...
        input = input.to("sdaa").to(memory_format=torch.channels_last)
        target = target.to("sdaa")
        # meta = meta.to("sdaa")
        target_weight = target_weight.to("sdaa")
        model = model.to("sdaa")
        ...
        with torch.sdaa.amp.autocast():
            ...
            target = target.to(device)  # device 已定义为 sdaa:0
