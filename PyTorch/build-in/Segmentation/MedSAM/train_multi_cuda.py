...
def init_env():
    ...
    parser.add_argument('--nproc_per_node', type=int, default=torch.cuda.device_count(), help='nproc_per_node')
...
def main_worker(local_rank, nprocs, args):
    ...
    print(f"[Rank {rank}]: Use cuda: {local_rank} for training")
    is_main_host = rank == 0
    if is_main_host:
    ...
    if args.use_amp:
        ...
        print(f"[RANK {rank}: cuda {local_rank}] Using AMP for training")
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for step, (image, gt_2d, boxes, _) in enumerate(
                tqdm(train_dataloader, desc=f"[Epoch: {epoch + 1}/{num_epochs}][RANK {rank}: cuda {local_rank}]")
        ):
            total_steps += 1
            optimizer.zero_grad()
            boxes_np = boxes.detach().cpu().numpy()

            image, gt_2d = image.cuda(), gt_2d.cuda()
            if args.use_amp:
