...
def init_env():
    ...
    parser.add_argument('--nproc_per_node', type=int, default=torch.sdaa.device_count(), help='nproc_per_node')
...
def main_worker(local_rank, nprocs, args):
    ...
    print(f"[Rank {rank}]: Use sdaa: {local_rank} for training")
    is_main_host = rank == 0
    if is_main_host:
    ...
    if args.use_amp:
        ...
        print(f"[RANK {rank}: sdaa {local_rank}] Using AMP for training")
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for step, (image, gt_2d, boxes, _) in enumerate(
                tqdm(train_dataloader, desc=f"[Epoch: {epoch + 1}/{num_epochs}][RANK {rank}: sdaa {local_rank}]")
        ):
            total_steps += 1
            optimizer.zero_grad()
            boxes_np = boxes.detach().cpu().numpy()

            image, gt_2d = image.sdaa(), gt_2d.sdaa()
            if args.use_amp:
            ...
            if is_main_host:
                with open(f"{model_save_path}/trai.log",'a',encoding='utf-8')as f:
                    f.write(f'[{time.asctime()}][RANK-{rank}][EPOCH-{epoch}][STEP-{total_steps}] '
                        f'Loss: {loss.detach().cpu().float()}\n')
                print(f'[{time.asctime()}][RANK-{rank}][EPOCH-{epoch}][STEP-{total_steps}] '
                    f'Loss: {loss.detach().cpu().float()}')

                sstep.append(total_steps)
