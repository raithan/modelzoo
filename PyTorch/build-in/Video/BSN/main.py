...
def train_TEM(data_loader,model,optimizer,epoch,writer,opt):
    ...
    for n_iter,(input_data,label_action,label_start,label_end) in enumerate(data_loader):
        ...
        print(
        "Step: {} | Epoch: {} | total_loss: {:.03f}".format(
        n_iter, epoch,
    ...
    print("TEM training loss(epoch %d): action - %.03f, start - %.03f, end - %.03f, total - %.03f" 
                                                                                        %(epoch + 1,epoch_action_loss/(n_iter+1),
...
def test_TEM(data_loader,model,epoch,writer,opt):
    ...
    if epoch_cost< model.tem_best_loss:
        model.tem_best_loss = np.mean(epoch_cost)
...
def test_PEM(data_loader,model,epoch,writer,opt):
    ...
    if epoch_iou_loss<model.pem_best_loss :
        model.pem_best_loss = np.mean(epoch_iou_loss)
...
def BSN_inference_TEM(opt):
    ...
    checkpoint = torch.load(opt["checkpoint_path"] + "/tem_best.pth.tar", weights_only=True)
