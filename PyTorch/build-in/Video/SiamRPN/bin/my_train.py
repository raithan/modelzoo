...
def setup_logger(log_file_path):
    """设置日志记录器"""
    logger = logging.getLogger('siamrpn_train')
...
def train(data_dir, resume_path=None, vis_port=None, init=None):
    ...
    if not os.path.exists('./logs/'):
        os.makedirs('./logs/')
    logger = setup_logger('./logs/siamrpn_train.log')
    ...
    for epoch in range(start_epoch, config.epoch + 1):
        ...
        for i, data in enumerate(tqdm(trainloader)):
            ...
            # 将loss信息记录到日志文件中
            loss_info = "[epoch %2d][iter %4d] cls_loss: %.4f, reg_loss: %.4f, total_loss: %.4f, lr: %.2e" % (
                epoch, i, cls_loss.detach().cpu().numpy(), reg_loss.detach().cpu().numpy(), 
                loss.detach().cpu().numpy(), optimizer.param_groups[0]['lr'])
            ...
            #if (i + 1) % 5 == 0:
                avg_loss_info = "[epoch %2d][iter %4d] cls_loss: %.4f, reg_loss: %.4f lr: %.2e" % (
                    epoch, i, loss_temp_cls / config.show_interval, loss_temp_reg / config.show_interval,
                    optimizer.param_groups[0]['lr'])
                tqdm.write(avg_loss_info)
                logger.info("AVG " + avg_loss_info)
                loss_temp_cls = 0
                loss_temp_reg = 0
                # if vis_port:
        ...
        epoch_info = "EPOCH %d valid_loss: %.4f, train_loss: %.4f" % (epoch, valid_loss, train_loss)
        print(epoch_info)
        ...
        if epoch % config.save_interval == 0:
            ...
            save_info = 'save model: {}'.format(save_name)
            print(save_info)
