...
#     """
#     def __init__(self, root_dir='/data/application/huangyun/SiamTrackers-master/SiamFC/SiamFC/got10k/datasets/got-10k', subset='test', return_meta=False):
...
#                 expected_path = os.path.join(self.root_dir, self.subset, index)
...
class GOT10k(object):
    ...
    Args:
        ...
        return_meta (string, optional): If True, returns ``meta``
            ...
            only returns ``img_files`` and ``anno``.
    """
    def __init__(self, root_dir='/data/application/huangyun/SiamTrackers-master/SiamRPN/SiamRPN/got10k/datasets/got-10k', subset='test', return_meta=False):
        super(GOT10k, self).__init__()
        assert subset in ['train', 'val', 'test'], 'Unknown subset.'
    ...
    def __getitem__(self, index):
        ...
        if isinstance(index, six.string_types):
            if not index in self.seq_names:
                expected_path = os.path.join(self.root_dir, self.subset, index)
                raise Exception('Sequence {} not found. Expected location: {}'.format(index, expected_path))
            index = self.seq_names.index(index)
    ...
    def _check_integrity(self, root_dir, subset):
        ...
        if os.path.isfile(list_file):
            ...
            for seq_name in seq_names:
                seq_dir = os.path.join(root_dir, subset, seq_name)
                if not os.path.isdir(seq_dir):
                    print('Warning: sequence %s not exists. Expected location: %s' % (seq_name, seq_dir))
        else:
            # dataset not exists
            raise Exception('Dataset not found or corrupted. Expected list file location: %s' % list_file)
