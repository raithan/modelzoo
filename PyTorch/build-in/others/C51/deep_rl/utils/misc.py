...
def run_steps(agent):
    ...
    t0 = time.time()
    if(config.pth_path != 'null' and config.status_path!='null' and config.eval_interval):
        with open(config.status_path, 'rb') as f:
            config.state_normalizer.load_state_dict(pickle.load(f))
        state_dict = torch.load(config.pth_path, map_location=lambda storage, loc: storage)
    ...
    while True:
        ...
        if config.log_interval and not agent.total_steps % config.log_interval:
            ...
            json_logger.log(
                    step = (0, agent.total_steps),
                    data = {
                    "rank": 0,
                    "train.ips": (config.log_interval / (time.time() - t0)),
                    },
