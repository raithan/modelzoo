...
def select_device(device, device_id):
    if device == 'use_gpu':
        if torch.cuda.is_available() and device_id >= 0:
            Config.DEVICE = torch.device('cuda:%d' % (device_id))
    elif device == 'use_npu':
        if torch.npu.is_available() and device_id >= 0:
            Config.DEVICE = torch.device('npu:%d' % (device_id))
