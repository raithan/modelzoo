...
class DQNActor(BaseActor):
    def __init__(self, config):
        BaseActor.__init__(self, config)  # 修正：添加了self参数
        self.config = config
        self.start()
...
class DQNAgent(BaseAgent):
    def __init__(self, config):
        BaseAgent.__init__(self, config)  # 修正：添加了self参数
        self.config = config
        config.lock = mp.Lock()

        # 创建梯度缩放器
        self.scaler = amp.GradScaler()  # 新增：PyTorch AMP梯度缩放器
    ...
    def step(self):
        ...
        if self.total_steps > self.config.exploration_steps:
            ...
            with amp.autocast():  # 新增：自动混合精度上下文
                loss = self.reduce_loss(loss)  # 重新计算loss（在FP16中）
            
            # 反向传播和优化步骤
            self.scaler.scale(loss).backward()  # 替换amp.scale_loss
            nn.utils.clip_grad_norm_(self.network.parameters(), self.config.gradient_clip)
            with config.lock:
                self.scaler.step(self.optimizer)  # 替换optimizer.step()
