""" Finetuning the library models for sequence classification with torch.cuda.amp """
...
def train(args, train_dataset, model, tokenizer, rank):
    """ Train the model with torch.cuda.amp for mixed precision training """
    args.train_batch_size = args.per_gpu_train_batch_size * max(1,0)
    train_sampler = DistributedSampler(train_dataset, rank=rank, num_replicas=args.world_size)
    ...
    # Initialize torch.cuda.amp for mixed precision training
    ...
    model.zero_grad()
    seed_everything(args.seed)  # Added for reproductibility
    steps_completed = 0
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if steps_completed >= num_training_steps:
                    print("run step is ",num_training_steps,"exit...")
                    exit()

            if rank == 0 and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                # Log metrics
                if args.local_rank == -1:  # Only evaluate when single GPU
                    evaluate(args, model.module, tokenizer)
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        args.eval_batch_size = args.per_gpu_eval_batch_size * max(1,0)
        ...
        for step, batch in enumerate(eval_dataloader):
            ...
            with torch.no_grad():
                ...
                # Mixed precision evaluation (optional, but can improve speed)
                with torch.cuda.amp.autocast(enabled=args.fp16):
                    outputs = model(**inputs)
...
def main_worker(rank, world_size, args):
    ...
    if rank == 0:
        logger.warning("Process rank: %s, device: %s, n_gpu: %s, distributed training: %s, 16-bits training: %s",
                       rank, args.device, args.n_gpu, bool(rank != -1), args.fp16)
    # Set seed
    ...
    if args.do_train:
        ...
        global_step, tr_loss = train(args, train_dataset, model, tokenizer, rank)
        if rank == 0:
            logger.info(" global_step = %s, average loss = %s", global_step, tr_loss)

    # Saving best-practices: if you use defaults names for the model, you can reload it using from_pretrained()
    if args.do_train and rank == 0:
...
def main():
    ...
    parser.add_argument('--fp16_opt_level', type=str, default='O1',
                        help="For fp16: Optimization level (not used with torch.cuda.amp, kept for compatibility)")
