...
def infer_one(model, wav_path: Path) -> Dict:
    """推理单个 wav，返回结果 dict"""
    # 语音必须 16 kHz 单声道
    speech, sr = sf.read(wav_path)
    if sr != 16000:
        raise RuntimeError(f"{wav_path} 采样率为 {sr} Hz，应先转换为 16k")
    return model(wav_path)
...
def main():
    ...
    if args.wav:
        ...
        print(f"# {args.wav.name} 语音段（秒）：", res["speech_segments"])
    else:
