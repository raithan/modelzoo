...
def apply_cmvn(inputs: torch.Tensor, cmvn: torch.Tensor) -> torch.Tensor:
    """
    Apply CMVN (Cepstral Mean and Variance Normalization) using pure PyTorch.

    Args:
        inputs (torch.Tensor): shape [T, D] or [B, T, D]
        cmvn (torch.Tensor): shape [2, D], mean and variance of training set
    ...
    if len(inputs.shape) == 3:
        B, T, D = inputs.shape
        inputs = inputs.view(-1, D)  # Flatten to [B*T, D]

    frame, dim = inputs.shape
    assert cmvn.shape[0] == 2, "CMVN must have two rows: mean and variance"
    assert dim == cmvn.shape[1], "Input dim must match CMVN dim"

    mean = cmvn[0:1]  # [1, D]
    var = cmvn[1:2]   # [1, D]

    # Expand mean and var to [frame, dim]
    inputs = inputs * var.type_as(inputs)
    inputs = inputs + mean.type_as(inputs)

    if len(inputs.shape) == 3:
        inputs = inputs.view(B, T, D)
    ...
    def forward_lfr_cmvn(
        ...
        **kwargs,
        ...
        for i in range(batch_size):
            mat = input[i, : input_lengths[i], :]
            if self.lfr_m != 1 or self.lfr_n != 1:
                mat, cache["lfr_splice_cache"][i], lfr_splice_frame_idx = WavFrontendOnline.apply_lfr(
...
from funasr.utils.tables import frontend_classes
frontend_classes["WavFrontendOnline"] = WavFrontendOnline