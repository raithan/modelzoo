...
def transform(im, quads, texts, normalizer, data_set):
    ...
    for quad_id, quad in enumerate(quads):
        ...
        if type(intersected_polygon) is Polygon and not intersected_polygon.is_empty:
            intersected_quad = np.array(intersected_polygon.exterior.coords[:-1])
            if intersected_quad.size > 0:
...
class SynthText(torch.utils.data.Dataset):
    def __init__(self, root, transform):
        ...
        total_samples = self.labels['imnames'].shape[1] // 105
        for i in range(total_samples):
            idx = (i * 105) + random.randint(0, 104)
            img_path = os.path.join(self.root, self.labels['imnames'][0, idx][0])
            if os.path.exists(img_path):
                self.valid_indices.append(i)
                if len(self.valid_indices) % 1000 == 0:
                    print(f"Found {len(self.valid_indices)} valid samples so far...")
        
        print(f"Dataset initialization complete. Found {len(self.valid_indices)} valid samples out of {total_samples} total samples.")

    def __len__(self):
        return len(self.valid_indices)

    def __getitem__(self, idx):
        actual_idx = (self.valid_indices[idx] * 105) + random.randint(0, 104)
        
        if actual_idx in self.broken_image_ids:
            for _ in range(10):
                new_idx = random.randint(0, len(self) - 1)
                new_actual_idx = (self.valid_indices[new_idx] * 105) + random.randint(0, 104)
                if new_actual_idx not in self.broken_image_ids:
                    actual_idx = new_actual_idx
                    idx = new_idx
                    break
        
        img_path = os.path.join(self.root, self.labels['imnames'][0, actual_idx][0])
        
        if not os.path.exists(img_path):
            print(f"Warning: File not found (should not happen): {img_path}")
            self.broken_image_ids.add(actual_idx)
        ...
        coordinates = self.labels['wordBB'][0, actual_idx]
        if len(coordinates.shape) == 2:
            coordinates = np.expand_dims(coordinates, axis=2)
        transposed = np.transpose(coordinates, (2, 1, 0))
        
        if not ((transposed > 0).all() and (transposed[:, :, 1] < img.shape[1]).all() and (transposed[:, :, 1] < img.shape[0]).all()):
            self.broken_image_ids.add(actual_idx)
            return self[random.randint(0, len(self) - 1)]
            
        if not ((transposed[:, 0] != transposed[:, 1]).all() and
            (transposed[:, 0] != transposed[:, 2]).all() and
            (transposed[:, 0] != transposed[:, 3]).all() and
            (transposed[:, 1] != transposed[:, 2]).all() and
            (transposed[:, 1] != transposed[:, 3]).all() and
            (transposed[:, 2] != transposed[:, 3]).all()):
                self.broken_image_ids.add(actual_idx)
                return self[random.randint(0, len(self) - 1)]
