...
#     handlers=[
#         logging.FileHandler("202506181600.log"),
...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            imgs, targets = imgs.to(device).to(memory_format=torch.channels_last), targets.to(device)
            # Warmup
            if ni <= nw:
                xi = [0, nw]  # x interp
            ...
            if rank in [-1, 0]:
                ...
                # if i % print_interval == 0 or i == len(dataloader) - 1:
                #   box_loss, obj_loss, cls_loss, total_loss = loss_items
                #   logger.info(f"\n[Epoch {epoch}/{epochs - 1}] [Batch {i}/{len(dataloader)}] "
                #               f"Loss: {total_loss.item():.4f} | "
                #               )
                box_loss, obj_loss, cls_loss, total_loss = loss_items
                logger.info(f"[Epoch {epoch}/{epochs - 1}] [Step {i}/{len(dataloader)}] "
                            f"Box Loss: {box_loss.item():.4f}, Obj Loss: {obj_loss.item():.4f}, "
                            f"Cls Loss: {cls_loss.item():.4f}, Total Loss: {total_loss.item():.4f}")
