...
def do_train(cfg, model, resume=False):
    ...
    with EventStorage(start_iter) as storage:
        ...
        for data, iteration in zip(data_loader, range(start_iter, max_iter)):
            ...
            storage.step()
            # data = [d.to(device).to(memory_format=torch.channels_last) for d in data]
            # iteration = iteration.to(device)

            for d in data:
                image = d["image"]
                if "instances" in d:
                    gt_boxes = d["instances"].gt_boxes.tensor
                    if len(gt_boxes) == 0:
                        logger.warning(f"Iter {iteration}: Empty ground truth boxes detected. Skipping this batch.")
                        continue
                if image.numel() == 0:
                    logger.warning(f"Iter {iteration}: Empty image tensor detected. Skipping this batch.")
                    continue
            ...
            if not torch.isfinite(losses).all():
                if comm.is_main_process():
                    log_msg = "[Loss Debug] Iter: {} | Loss is NaN/Inf. Details:\n".format(iteration)
                    for k, v in loss_dict.items():
                        log_msg += "  {}: {}\n".format(k, v.detach().cpu().numpy() if v.dim() == 0 else v.shape)
                    logger.error(log_msg)

                raise FloatingPointError("Loss became infinite or NaN!")            
