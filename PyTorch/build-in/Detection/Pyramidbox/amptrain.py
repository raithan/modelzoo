...
device = 'sdaa' if (torch.cuda.is_available() and args.cuda) else 'cpu'
    ...
    for epoch in range(start_epoch, cfg.EPOCHES):
        ...
        for batch_idx, (images, face_targets, head_targets) in enumerate(train_loader):
            if args.cuda:
                ...
                images = images.to(device).to(memory_format=torch.channels_last)
                face_targets = [ann.to(device) for ann in face_targets]
                head_targets = [ann.to(device) for ann in head_targets]
            else:
            ...
            logging.info(f'Epoch: {epoch} || Iteration: {iteration} || '
                        f'Total Loss: {loss_:.4f} || Face Loss: {face_loss:.4f} || '
                        f'Head Loss: {head_loss:.4f} || LR: {optimizer.param_groups[0]["lr"]:.6f}')            
