...
def parse_string(string):
    pattern = r"loss:(\s*[\d\.eE+-]+)"
    matches = re.findall(pattern, string)
    return [m.strip() for m in matches]

def parse_loss(ret_list):
    loss_arr = np.array([float(loss) for loss in ret_list], dtype=np.float32)
    print("Parsed loss array:", loss_arr[:10])
    return loss_arr
...
if __name__=="__main__":
    ...
    parser.add_argument('--sdaa-log', type=str,default="/localnvme/application/huangyun/rcf/202506191601.log")
    parser.add_argument('--cuda-log', type=str,default="/localnvme/application/huangyun/rcf/202506191604cuda.log")
