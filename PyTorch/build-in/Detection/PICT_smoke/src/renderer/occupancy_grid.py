        ...
        for cas in range(self.cascade):
            ...
            while head < C:
                ...
                if given_mask is not None:
                    ...
                    gt_masks = torch.from_numpy(gt_masks).to(decice)
...
class OccupancyGridDynamic():
    ...
    def debug_vis(self, poses, intrinsics, pts = None, pts_mask = None):
        ...
        if pts is not None:
            ...
            cas_world_xyzs = torch.from_numpy(cas_world_xyzs).float().to(decice)
...
def init_occ_grid(args, model, poses = None, intrinsics = None, given_mask = None):
    ...
    if "hybrid_neus" in args.net_model:
        model.occupancy_grid_dynamic.mark_untrained_grid(poses, intrinsics, given_mask, S = 2)
        ...
        model.occupancy_grid_static.mark_untrained_grid(poses[:,0,:,:], intrinsics[:,0,:,:], None, S = 2)
    else:
