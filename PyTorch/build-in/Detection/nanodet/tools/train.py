...
if hasattr(torch, 'cuda') and torch.cuda.is_available():
    ...
    torch.cuda.get_device_name = lambda dev: "CUDA"
    torch.cuda.get_device_capability = lambda dev: (7, 0)

    if not hasattr(torch._C, '_cuda_clearCublasWorkspaces'):
        torch._C._cuda_clearCublasWorkspaces = lambda: None
    if not hasattr(torch._C, '_cuda_resetAccumulatedErrors'):
        torch._C._cuda_resetAccumulatedErrors = lambda dev: None
